...
const seed = async () => {
    try {
        ...
        console.log("🗑 Clearing existing streams...");
        await Stream.deleteMany({});
        ...
        const demoStreams = [
            ...
            {
                title: "Valorant Tournament",
                description: "Live esports match",
                category: "gaming",
                isLive: false,
                viewerCount: 820,
                startedAt: new Date(Date.now() - 3600 * 1000),
            },
            {
                ...
                description: "Lo-fi beats and guitar",
                category: "music",
                isLive: true,
                viewerCount: 3000,
                startedAt: new Date(),
        ...
        console.log("📥 Inserting streams...");
        const result = await Stream.insertMany(demoStreams);

        console.log(`✅ Seed complete — Inserted ${result.length} streams`);
        process.exit(0);
    } catch (error) {
